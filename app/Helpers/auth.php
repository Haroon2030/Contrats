...
$auxiliary_page_permissions = [
    'search_supplier' => ['add_contract', 'add_items'],
    'get_user_permissions' => ['users'],
];

if (isset($auxiliary_page_permissions[$page])) {
    if (vcUserHasAnyPagePermission($conn, $uid, $auxiliary_page_permissions[$page])) {
        return;
    }
...
if ($jobRole === 'commercial_manager') {
    if (vcCommercialManagerPageAllowed($page) || vcUserHasPagePermission($conn, $uid, $page)) {
        return;
    }
